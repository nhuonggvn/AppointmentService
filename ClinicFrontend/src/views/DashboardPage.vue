                    ...
                    <v-card border flat class="bg-surface pa-6 mb-6">
                      <div class="d-flex justify-space-between align-center mb-4">
                        ...
                        <div class="d-flex gap-2">
                          ...
                          />
                          <v-btn
                            v-if="selectedDoctorIds.length > 0"
                            color="error"
                            prepend-icon="mdi-trash-can"
                            class="font-weight-bold"
                            @click="deleteSelectedDoctors"
                          >
                            Xóa đã chọn ({{ selectedDoctorIds.length }})
                          </v-btn>
...
export default {
  ...
  setup() {
    ...
    // --- INTEGRATED FROM DEMO.VUE FOR PHARMACY, BILLING & FINANCIAL REPORTS ---
    ...
    // Doctor multi-selection & bulk deletion logic
    const selectedDoctorIds = ref([])
    
    const isAllDoctorsSelected = computed({
      get: () => {
        if (paginatedDoctors.value.length === 0) return false
        return paginatedDoctors.value.every(d => selectedDoctorIds.value.includes(d.id))
      },
      set: (val) => {
        if (val) {
          paginatedDoctors.value.forEach(d => {
            if (!selectedDoctorIds.value.includes(d.id)) {
              selectedDoctorIds.value.push(d.id)
            }
          })
        } else {
          paginatedDoctors.value.forEach(d => {
            const idx = selectedDoctorIds.value.indexOf(d.id)
            if (idx !== -1) {
    ...
    const deleteSelectedDoctors = async () => {
      if (selectedDoctorIds.value.length === 0) return
      if (!confirm(`Bạn có chắc chắn muốn ngừng kích hoạt ${selectedDoctorIds.value.length} bác sĩ đã chọn?`)) return
      try {
        loading.value = true
        if (isMockMode.value) {
          selectedDoctorIds.value.forEach(id => {
            const doc = doctors.value.find(d => d.id === id)
            if (doc) doc.isActive = false
          })
          showAlert('Đã thay đổi trạng thái các bác sĩ (Giả lập)', 'success')
        } else {
          for (const id of selectedDoctorIds.value) {
            const url = `${apiUrl.value}/doctors/${id}`
            const res = await fetch(url, {
              method: 'DELETE',
              headers: {
            ...
            if (!res.ok) console.error(`Lỗi ngừng kích hoạt bác sĩ ID ${id}`)
          }
          await fetchDoctors()
          showAlert('Đã ngừng kích hoạt các bác sĩ đã chọn thành công!', 'success')
        }
